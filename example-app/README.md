...
- 📄 Скопируйте `.env.exmample` в `.env` (см. [таблицу с переменными](#env-vars)). В файле [docker-compose.yaml](docker-compose.yaml) представлены два контейнера `database`: mariadb (рабочий) и mysql (закомментирован), можете ничего не менять и тогда СУБД будет mariadb, но мждете закомментировать mariadb и расскомментировать mysql чтобы СУБД проекта была mysql.
