...
2. На странице `Создание базы данных` указать следующие данные: `Сервер` (название вашего контейнера СУБД; значение из
   переменной [DB_HOST](#DB_HOST)), `Имя пользователя` (значение из переменной
   [DB_USER](#DB_USER)), `Пароль` (значение из переменной
   [DB_PASSWORD](#DB_PASSWORD)), `Имя базы данных` (значение из переменной [DB_DATABASE](#DB_DATABASE)). Больше ничего менять не надо.
...
4. После установки БУС вы попадете на главную страницу вашего сайта с предустановленным шаблоном. Для перехода в
   админ-панель БУС перейдите на страницу `http://localhost/bitrix` или `http://APP_HOST/bitrix` (хост — [APP_HOST](#APP_HOST)).
